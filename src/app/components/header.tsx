...
interface HeaderProps {
    logoLink: LinkField;
    menuItems: { menu_link: LinkField; menu_label: KeyTextField }[];
}

const AppHeader: React.FC<HeaderProps> = ({ logoLink, menuItems }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            gsap.fromTo(menuRef.current, 
                ...
                { y: '0%', opacity: 1, display: 'flex', duration: 0.5, ease: 'power2.out' }
            );
        } else {
            gsap.to(menuRef.current, 
                { y: '-100%', opacity: 0, display: 'none', duration: 0.5, ease: 'power2.in' }
            );
        }
    }, [isOpen]);
    ...
    return (
        <header className={`text-white absolute top-0 left-0 w-full z-50 ${isOpen ? 'backdrop-blur-lg' : ''}`}>
            <div className="flex items-center justify-between px-8">
            
                <PrismicNextLink field={logoLink} className="flex items-center mb-2">
                    <div className="w-32 h-auto md:w-40 lg:w-48">
                        <svg xmlns="http://www.w3.org/2000/svg" width="128" height="75" viewBox="0 0 844 102" fill="none">
                ...
                <div className="hidden md:flex gap-5 text-base">
                    {Array.isArray(menuItems) && menuItems.map((menuItem, index) => (
                        <PrismicNextLink key={index} field={menuItem.menu_link} className="hover:text-text-gray-on-black">
                            {menuItem.menu_label}
                        </PrismicNextLink>
                    ))}
                </div>
            ...
            <div ref={menuRef} className="md:hidden flex-col gap-2 px-8 pb-4 mt-0 backdrop-blur-lg overflow-hidden" style={{ display: 'none' }}>
                {Array.isArray(menuItems) && menuItems.map((menuItem, index) => (
                    <PrismicNextLink key={index} field={menuItem.menu_link} onClick={toggleMenu} className="hover:text-text-gray-on-black">
                        {menuItem.menu_label}
                    </PrismicNextLink>
                ))}
            </div>
